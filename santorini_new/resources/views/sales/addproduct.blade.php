                        ...
                        <form method="post" action="{{ route('sales.product.store', $sale) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-product">Produtos</label>
...
{{--                                                <option value="{{$product['id']}}" selected>[{{ $product->category->name }}] {{ $product->name }} - Preço base: {{ $product->price }} MT</option>--}}
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-total">Montante total</label>
                                    <input type="text" name="total_amount" id="input-total" class="form-control form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" value="0MT" disabled>
                                    @include('alerts.feedback', ['field' => 'product_id'])
                                </div>
